...
(defun read-inputs (filename)
  (with-temp-buffer
    ...
    (setq signals (mapcar 'string-to-number (split-string (string-trim (buffer-string)) "\n")))))

(read-inputs "input.txt")

(defun growing-signal? (before after)
  (< before after))

(defun find-growing-signals (signals)
  (mapcar* 'growing-signal? (butlast signals) (cdr signals)))

(defun count-deeper-signals (signals)
  (length (cl-remove-if nil (find-growing-signals signals))))

(count-deeper-signals '(199 200 208 210 200 207 240 269 260 263))
